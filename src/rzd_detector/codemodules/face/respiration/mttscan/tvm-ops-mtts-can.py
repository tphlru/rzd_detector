...
def _check_data_format(keras_layer):
    if (
        hasattr(keras_layer, ("data_format"))
        and keras_layer.data_format != "channels_last"  # noqa:W503
    ):
        raise ValueError(
...
def _convert_attention_mask(inexpr, keras_layer, etab):
    xsum = _op.reduce.sum(
        _op.reduce.sum(inexpr, axis=2, keepdims=True), axis=3, keepdims=True
    )
    xshape = infer_shape(inexpr)
    return (
        (inexpr)
        / xsum  # noqa:W503
        * tvm.relay.expr.const(xshape[2], dtype="float32")  # noqa:W503
        * tvm.relay.expr.const(xshape[3], dtype="float32")  # noqa:W503
        * tvm.relay.expr.const(0.5, dtype="float32")  # noqa:W503
    )
...
def _convert_activation(inexpr, keras_layer, _):
    ...
    if act_type == "linear":
        return _extracted_from__convert_activation_9(keras_layer, inexpr)
    if act_type == "softmax":
    ...
    if act_type == "hard_sigmoid":
        x = (_expr.const(0.2, dtype="float32") * inexpr) + _expr.const(
            0.5, dtype="float32"
        )
...
# TODO Rename this here and in `_convert_activation`
def _extracted_from__convert_activation_9(keras_layer, inexpr):
    if isinstance(keras_layer, str):
        return inexpr
    alpha = keras_layer.alpha if hasattr(keras_layer, "alpha") else 1.0
...
def _convert_advanced_activation(inexpr, keras_layer, etab):
    ...
    if act_type == "PReLU":
        ...
        alpha = etab.new_const(
            keras_layer.get_weights()[0].transpose(np.roll(range(size), 1))
        )
        return _op.negative(alpha) * _op.nn.relu(_op.negative(inexpr)) + _op.nn.relu(
            inexpr
        )
    if act_type == "ThresholdedReLU":
        ...
        return _op.multiply(
            inexpr,
            _op.greater(inexpr, _expr.const(theta, dtype="float32")).astype("float32"),
        )
...
def _convert_merge(inexpr, keras_layer, _):
    ...
    ret = inexpr[0]
    if merge_type == "Dot":
        ret = _extracted_from__convert_merge_(keras_layer, inexpr)
...
# TODO Rename this here and in `_convert_merge`
def _extracted_from__convert_merge_(keras_layer, inexpr):
    ...
    return _op.transpose(ret_dot, axes=[0, 2, 1])
...
def _convert_dense(inexpr, keras_layer, etab):
    ...
    if input_dim > 2:
        input_shape = tuple(dim or 1 for dim in _as_list(input_shape)[0])
...
def _convert_convolution(inexpr, keras_layer, etab):
    ...
    if etab.data_layout == "NHWC":
        kernel_layout = "HWOI" if is_depthconv else "HWIO"
    else:
        kernel_layout = "OIHW"
    ...
    params = {
        "weight": etab.new_const(weight, dtype=str(weight.dtype)),
...
def _convert_separable_convolution(inexpr, keras_layer, etab):
    _check_data_format(keras_layer)
    kernel_layout = "HWOI" if etab.data_layout == "NHWC" else "OIHW"
    weightList = keras_layer.get_weights()
    # depthwise conv
...
def _convert_upsample3d(inexpr, keras_layer, etab):
    ...
    params = {"scale_d": d, "scale_h": h, "scale_w": w, "layout": etab.data_layout}
    return _op.nn.upsampling3d(inexpr, **params)
...
def _convert_batchnorm(inexpr, keras_layer, etab):
    ...
    params["beta"] = (
        etab.new_const(np.zeros(moving_mean.shape))
        if "beta" not in params
        else params["beta"]
    )
    params["gamma"] = (
        etab.new_const(np.ones(moving_mean.shape))
...
def _convert_padding(inexpr, keras_layer, etab):
    ...
    if etab.data_layout == "NCHW":
        return _op.nn.pad(
            data=inexpr, pad_width=((0, 0), (0, 0), (top, bottom), (left, right))
        )
    return _op.nn.pad(
        data=inexpr, pad_width=((0, 0), (top, bottom), (left, right), (0, 0))
    )
...
def _convert_padding3d(inexpr, keras_layer, etab):
    ...
    if isinstance(padding, tuple) and isinstance(padding[0], tuple):
        ...
        msg = (
            'Value {} in attribute "padding" of operator ZeroPadding3D is ' "not valid."
        )
        raise tvm.error.OpAttributeInvalid(msg.format(str(padding)))
...
def _convert_reshape(inexpr, keras_layer, etab):
    ...
    if len(inshape) == 3 and len(tshape) == 1:
        ...
        assert (
            tshape[0] == tshape[1]
        ), f"Only supports square target shapes, but got {tshape}"
        shape = (-1,) + tshape
    else:
        # (?, h, w, c) -> (-1, c, H, W)
        # (?, h, w, c) -> (-1, c, hw)
...
def _convert_lstm(inexpr, keras_layer, etab):
    ...
    next_c = inexpr[2]
    weightList = keras_layer.get_weights()
    in_shape = tuple(dim or 1 for dim in _as_list(keras_layer.input_shape)[0])
    ...
    for data in in_data:
        ...
        ixh2 = _op.nn.bias_add(
            _op.nn.dense(next_h, recurrent_weight, units=units), bias=in_bias
        )
        gate = ixh1 + ixh2
...
def _convert_simple_rnn(inexpr, keras_layer, etab):
    ...
    ixh = _op.nn.bias_add(
        _op.nn.dense(in_data, kernel_weight, units=units), bias=in_bias
    )
    ...
    out_shape = tuple(
        dim or 1 for dim in _as_list(keras_layer.output_shape)[0]
    )
...
def _convert_gru(inexpr, keras_layer, etab):
    ...
    units = list(weightList[0].shape)[1]
    in_data = _op.nn.batch_flatten(in_data)
    matrix_x = _op.nn.bias_add(
        _op.nn.dense(in_data, kernel_weight, units=units), in_bias
    )
    ...
    output = (
        rec_act_z * h_tm1_op + (_expr.const(1.0, dtype="float32") - rec_act_z) * act_hh
    )
    out_shape = tuple(
        dim or 1 for dim in _as_list(keras_layer.output_shape)[0]
    )
...
def from_keras(model, shape=None, layout="NCHW", dtype="float32"):
    ...
    assert layout in [
        ...
        "NHWC",
        "NDHWC",
    ], "Layout must be one of 'NCHW', NHWC or NDHWC"
    ...
    for keras_layer in model.layers:
        if isinstance(keras_layer, input_layer_class):
            ...
            if (
                len(shape) == 2
                and "conv2d" in keras_layer.name
                or len(shape) == 2
                and "pooling2d" in keras_layer.name
            ):
            ...
            if inbound_nodes is None:
                raise TypeError(
                    "Unknown layer type or unsupported Keras version : {}".format(
                        keras_layer
                    )
                )
            for node_idx, node in enumerate(inbound_nodes):
                ...
                if (
                    not is_tf_keras
                    and model._node_key(keras_layer, node_idx) not in model._network_nodes
                ):
                    continue
                inexpr = []
                ...
                for n_idx, t_idx, inbound_layer in zip_node:
                    if isinstance(inbound_layer, input_layer_class):
                        ...
                        expr_name = f"{inbound_layer.name}:{str(n_idx)}:{str(t_idx)}"
                ...
                keras_op_to_relay(
                    inexpr,
                    keras_layer,
                    f"{keras_layer.name}:{str(node_idx)}",
                    etab,
                )
    ...
    outexpr = [
        etab.get_expr(f"{oc[0].name}:{str(oc[1])}:{str(oc[2])}")
    ...
    params = {
        k: _nd.array(np.array(v, dtype=np.float32)) for k, v in etab.params.items()
    }
