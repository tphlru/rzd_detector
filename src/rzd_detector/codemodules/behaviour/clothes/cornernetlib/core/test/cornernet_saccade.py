...
def att_nms(atts, ks):
    ...
    pools = [
        nn.functional.max_pool2d(att, (k, k), stride=1, padding=pad)
        for k, att, pad in zip(ks, atts, pads)
    ]
...
def batch_decode(db, nnet, images, no_att=False):
    ...
    detections = (
        np.concatenate(detections, axis=0) if detections else np.zeros((0, num_dets, 8))
    )
...
def prepare_images(db, image, locs, flipped=True):
    ...
    for ind, (y, x, scale) in enumerate(locs[:, :3]):
        ...
        offsets[ind] = crop_image_gpu(
            image, [int(y), int(x)], [crop_height, crop_width], images[ind]
        )
...
def cornernet_saccade_inference(db, nnet, image, decode_func=batch_decode):
    ...
    im_mean = torch.tensor(db.mean, dtype=torch.float32, device="cuda").reshape(
        1, 3, 1, 1
    )
    # im_std = torch.cuda.FloatTensor(db.std).reshape(1, 3, 1, 1)
    im_std = torch.tensor(db.std, dtype=torch.float32, device="cuda").reshape(
        1, 3, 1, 1
    )
    ...
    for j in range(categories):
        ...
        keep_inds = soft_nms(
            top_bboxes[j + 1], Nt=nms_threshold, method=nms_algorithm, sigma=0.7
        )
